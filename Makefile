...
UPROGS  := hello conserver client fs ls cat run sh write rm cp mem deep forkd trap kill upper wc fp mkdir map rmdir mv
...
sdtest: all | $(BUILD)
	...
	@DEV=$$(hdiutil attach -nomount -imagekey diskimage-class=CRawDiskImage \
	 ...
	 mkdir -p "$$D/USR/BIN"; \
	 for p in hello ls cat run write rm cp mem deep forkd trap kill upper wc fp mkdir map rmdir mv; do \
